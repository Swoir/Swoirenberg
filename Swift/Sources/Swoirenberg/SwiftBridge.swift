...
public func prove_swift<GenericToRustStr: ToRustStr, GenericIntoRustString: IntoRustString>(_ circuit_bytecode: GenericIntoRustString, _ initial_witness: RustVec<Int64>, _ proof_type: GenericIntoRustString, _ srs_path: Optional<GenericToRustStr>) -> Optional<Proof> {
    return optionalRustStrToRustStr(srs_path, { srs_pathAsRustStr in
        { let val = __swift_bridge__$prove_swift({ let rustString = circuit_bytecode.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), { let val = initial_witness; val.isOwned = false; return val.ptr }(), { let rustString = proof_type.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), srs_pathAsRustStr); if val != nil { return Proof(ptr: val!) } else { return nil } }()
    })
}
public func verify_swift<GenericIntoRustString: IntoRustString, GenericToRustStr: ToRustStr>(_ circuit_bytecode: GenericIntoRustString, _ proof: RustVec<UInt8>, _ vkey: RustVec<UInt8>, _ proof_type: GenericIntoRustString, _ srs_path: Optional<GenericToRustStr>) -> Optional<Bool> {
    return optionalRustStrToRustStr(srs_path, { srs_pathAsRustStr in
        __swift_bridge__$verify_swift({ let rustString = circuit_bytecode.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), { let val = proof; val.isOwned = false; return val.ptr }(), { let val = vkey; val.isOwned = false; return val.ptr }(), { let rustString = proof_type.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), srs_pathAsRustStr).intoSwiftRepr()
    })
}
