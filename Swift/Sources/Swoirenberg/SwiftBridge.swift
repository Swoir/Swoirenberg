...
public func setup_srs_swift<GenericToRustStr: ToRustStr, GenericIntoRustString: IntoRustString>(_ circuit_bytecode: GenericIntoRustString, _ srs_path: Optional<GenericToRustStr>) -> Optional<UInt32> {
...
public func prove_swift<GenericIntoRustString: IntoRustString>(_ circuit_bytecode: GenericIntoRustString, _ initial_witness: RustVec<GenericIntoRustString>, _ proof_type: GenericIntoRustString, _ num_points: UInt32) -> Optional<Proof> {
...
public func verify_swift<GenericIntoRustString: IntoRustString>(_ proof: RustVec<UInt8>, _ vkey: RustVec<UInt8>, _ proof_type: GenericIntoRustString, _ num_points: UInt32) -> Optional<Bool> {
    __swift_bridge__$verify_swift({ let val = proof; val.isOwned = false; return val.ptr }(), { let val = vkey; val.isOwned = false; return val.ptr }(), { let rustString = proof_type.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), num_points).intoSwiftRepr()
}
